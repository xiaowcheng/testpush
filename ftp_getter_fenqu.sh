...
FLUME_FENQU_DIR=$HOME/fenqu
FLUME_YD12580_DIR=$HOME/yd12580
...
mkdir -p $LOGDIR
mkdir -p $FLUME_FENQU_DIR
mkdir -p $FLUME_YD12580_DIR
...
do
	...
	if [[ $linenum -gt 0 ]]; then
		if [[ $linenum -gt $TXTLINE ]]; then
	 		split -l $TXTLINE $DOWNLOAD_DIR/$a -d -a 4 $FLUME_FENQU_DIR"/$(date +%s%N)_"
			split -l $TXTLINE $DOWNLOAD_DIR/$a -d -a 4 $FLUME_YD12580_DIR"/$(date +%s%N)_"
	 	else
	 		cp $DOWNLOAD_DIR/$a $FLUME_FENQU_DIR"/$(date +%s%N)_0000"
			cp $DOWNLOAD_DIR/$a $FLUME_YD12580_DIR"/$(date +%s%N)_0000"
	 	fi 
	fi
done
